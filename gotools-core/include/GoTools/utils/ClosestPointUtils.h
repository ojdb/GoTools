...
struct StatusUpdater
{
    ...
    void updateStatusFile()
    {
	std::ofstream fileout_status(fileout_status_.c_str());
	if (curr_perc_local_ == 0)
	{
	    curr_perc_local_ = 1;
	}
	else if (curr_perc_local_ == 100)
	{
	    curr_perc_local_ = 99;
	}
	...
	std::cout << "Adding timestamp to status file (% completion & seconds since start): " <<
	    curr_perc_local_ << " " << time_diff << std::endl;
...
namespace Go
{
  ...
  std::vector<float> closestPointCalculations(const std::vector<float>& pts, const shared_ptr<boxStructuring::BoundingBoxStructure>& structure,
					      ...
					      StatusUpdater* status_updater = NULL);
  ...
  std::vector<float> closestPointCalculations(const std::vector<float>& pts, const shared_ptr<boxStructuring::BoundingBoxStructure>& structure,
					      const std::vector<std::vector<double> >& rotationMatrix, const Point& translation,
					      int return_type, StatusUpdater* status_updater = NULL);
