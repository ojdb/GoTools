...
namespace Go
{
					 ...
					 StatusUpdater* status_updater)
  {
	...
	if (status_updater != NULL)
	{
	    ...
	    double step = (double)(status_updater->perc_max_ - status_updater->perc_min_)/(double)(num_evals);
	    int curr_perc_local = status_updater->curr_perc_ + floor(cntr*step);
	    if (cntr%50000 == 0)
	    {
		cout << "num_evals: " << num_evals << ", step: " << step << ", curr_perc_local: " << curr_perc_local << endl;
	    }
  ...
  vector<float> closestSignedDistances(const vector<float>& inPoints, const shared_ptr<BoundingBoxStructure>& boxStructure,
				       ...
				       StatusUpdater* status_updater)
  {
      return closestPointCalculations(inPoints, boxStructure, rotationMatrix, translation, 1, status_updater);
  }
  ...
  vector<float> closestPoints(const vector<float>& inPoints, const shared_ptr<BoundingBoxStructure>& boxStructure,
			      ...
			      StatusUpdater* status_updater)
  {
      return closestPointCalculations(inPoints, boxStructure, rotationMatrix, translation, 2, status_updater);
  }
